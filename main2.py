...
if __name__ == "__main__":
    ...
    for j in range(time):
        ...
        contents_train_list=[]#储存所有处理好的的txt对应的句子  是 列表
        contents_trainham_list=[]
        contents_trainspam_list=[]
        ...
        for i in train_hamlist:
            ...
            contents_trainham_list.append(dict1['content'])
            info_train_hamlist[i]=dict1
        for i in train_spamlist:
            dict1={}
            dict1['wordslist'],dict1['content']=wf.WordsListofFile(i,trainpath)
            contents_train_list.append(dict1['content'])
            contents_trainspam_list.append(dict1['content'])
            info_train_spamlist[i]=dict1
        allwordslist=tfidf.getallword(contents_train_list,repeat=False)#获得所有单词  不重复
        allwords_trainhamlist_r=tfidf.getallword(contents_trainham_list,repeat=True)
        allwords_trainspamlist_r=tfidf.getallword(contents_trainspam_list,repeat=True)
        # print(allwordslist)
        allwordslist_r=tfidf.getallword(contents_train_list,repeat=True)#总单词
        ...
        for i in range(len(test_hamlist)):
            ...
            #1.伯努利模型 联合概率
            okemail,pswdict,p=bayes.bernoulli(ham,filename,info_train_spamlist,info_train_hamlist,allwordslist,Threshold,usetfidf=True)
            if okemail:
                #如果检测正确
                bcounthamok+=1
            else:#检测失败
                with open('mylog.txt','a')as f:
                    f.write("\nbernoulliFail:"+test_hamlist[i])
                    f.write('\n{0}\n'.format(pswdict))
                print('\nbernoulliFail：'+test_hamlist[i]+':',p)#输出文件名
                print(pswdict)
            #2.多项式模型 贝叶斯估计
            okemail,p=bayes.polynomial(ham,filename,allwords_trainhamlist_r,allwords_trainspamlist_r,allwordslist_r,usetfidf=True,uselapras=True)
            if okemail:
                #如果检测正确
                pcounthamok+=1
            else:#检测失败
                with open('mylog.txt','a')as f:
                    f.write("\npolynomialFail:"+test_hamlist[i])
                    # f.write('\n{0}\n'.format(pswdict))
                print('\npolynomialFail：'+test_hamlist[i]+':',p)#输出文件名
                # print(pswdict)
        for i in range(len(test_spamlist)):
            filename=test_spamlist[i]
            spam=info_test_spamlist[filename]
            #bonuli
            okemail,pswdict,p=bayes.bernoulli(spam,filename,info_train_spamlist,info_train_hamlist,allwordslist,Threshold,usetfidf=True)
            if okemail==False:
                bcountspamok+=1
            else:#检测失败
                with open('mylog.txt','a')as f:
                    f.write("\nbernoulliFail:"+test_spamlist[i])
                    f.write('\n{0}\n'.format(pswdict))
                print('\nbernoulliFail：'+test_spamlist[i]+':',p)#输出文件名
                print(pswdict)
            #duoxiangshi 
            okemail,p=bayes.polynomial(spam,filename,allwords_trainhamlist_r,allwords_trainspamlist_r,allwordslist_r,usetfidf=True,uselapras=True)
            if okemail==False:
                #如果检测正确
                pcounthamok+=1
            else:#检测失败
                with open('mylog.txt','a')as f:
                    f.write("\npolynomialFail:"+test_spamlist[i])
                    # f.write('\n{0}\n'.format(pswdict))
                print('\npolynomialFail：'+test_spamlist[i]+':',p)#输出文件名
                # print(pswdict)
        bAccuracy[j]=(bcounthamok+bcountspamok)/sumoftest
        pAccuracy[j]=(pcounthamok+pcountspamok)/sumoftest
        print('正确率：伯努利模型'+str(bAccuracy[j]),'多项式模型'+str(pAccuracy[j]))#计算正确性
        bsum_accur+=bAccuracy[j]
        psum_accur+=pAccuracy[j]
    # 
    baverage_accur=bsum_accur/time
    paverage_accur=psum_accur/time
    print("平均正确率：\n伯努利模型：", baverage_accur)
    print("多项式模型：",paverage_accur)
    with open('mylog.txt','a')as f:
        f.write("平均正确率：伯努利：{0}多项式：{1}\n".format(baverage_accur,paverage_accur))
    x=[x for x in range(1,time+1)]
    by=bAccuracy[:]
    py=pAccuracy[:]


    plt.scatter(x,by,c='b')
    plt.scatter(x,py,c='g')
